...
const EQUIPAMENTO_OPTIONS = [
  'Empilhadeira Diesel 16T',
  'Empilhadeira Diesel 7T',
  'Empilhadeira Diesel 10T',
...
export default function EstrForm() {
  const [status, setStatus] = useState('idle')
  const [equipamento, setEquipamento] = useState('')
  const navigate = useNavigate()

  const whatsappTais = buildWhatsappHref(WHATSAPP_TAIS, equipamento)
  const whatsappJoice = buildWhatsappHref(WHATSAPP_JOICE, equipamento)

  async function handleSubmit(e) {
    e.preventDefault()
    setStatus('sending')
    const data = { ...Object.fromEntries(new FormData(e.target).entries()), origem: 'LP Equipamentos Estratégicos' }
    try {
      await sendContactEmail(data)
      navigate('/lp/equipamentos-estrategicos/obrigado', { state: { equipamento } })
    } catch {
      setStatus('error')
    }
  }
    ...
    <section className="py-16 md:py-24 bg-eqp-ink bg-pattern relative" id="orcamento">
      <div className="max-w-7xl mx-auto px-4 md:px-6 grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="flex flex-col h-full">
          <h2 className="eqp-h1 text-eqp-chalk mb-6">Pronto para reforçar sua operação?</h2>
          <p className="eqp-body-lg text-eqp-fog mb-8">
            Nossa equipe comercial está pronta para analisar sua demanda e apresentar a melhor condição de locação para o equipamento que a sua operação precisa.
          </p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 gap-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label htmlFor="estr_nome" className={LABEL_CLASS}>Nome Completo</label>
                <input id="estr_nome" name="nome" type="text" required placeholder="Seu nome" className={INPUT_CLASS} />
              </div>
              <div className="space-y-1">
                <label htmlFor="estr_empresa" className={LABEL_CLASS}>Empresa</label>
                <input id="estr_empresa" name="empresa" type="text" required placeholder="Nome da empresa" className={INPUT_CLASS} />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label htmlFor="estr_telefone" className={LABEL_CLASS}>Telefone / WhatsApp</label>
                <input id="estr_telefone" name="telefone" type="tel" required placeholder="(00) 00000-0000" className={INPUT_CLASS} />
              </div>
              <div className="space-y-1">
                <label htmlFor="estr_email" className={LABEL_CLASS}>E-mail Corporativo</label>
                <input id="estr_email" name="email" type="email" required placeholder="email@empresa.com.br" className={INPUT_CLASS} />
              </div>
            </div>
            <div className="space-y-1">
              <label htmlFor="estr_equipamento" className={LABEL_CLASS}>Equipamento</label>
              <select
                id="estr_equipamento"
                name="servico"
                required
            ...
            <div className="space-y-1 flex-1 flex flex-col">
              <label htmlFor="estr_mensagem" className={LABEL_CLASS}>Detalhes da Demanda</label>
              <textarea
                id="estr_mensagem"
                name="mensagem"
